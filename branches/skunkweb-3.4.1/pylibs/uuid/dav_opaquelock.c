...
typedef unsigned64_t sw_uuid_time_t;

const sw_uuid_t null_locktoken = {0};

static void format_uuid_v1(sw_uuid_t * uuid, unsigned16 clockseq, sw_uuid_time_t timestamp, uuid_node_t node);
static void get_current_time(sw_uuid_time_t * timestamp);
static unsigned16 true_random(void);
static void get_pseudo_node_identifier(uuid_node_t *node);
static void get_system_time(sw_uuid_time_t *sw_uuid_time);
 ...
 */
int dav_create_opaquelocktoken(uuid_state *st, sw_uuid_t *u)
{
    sw_uuid_time_t timestamp;
...
char *dav_format_opaquelocktoken(const sw_uuid_t *u)
{
    char* buf;
    buf = malloc(37);
 ...
 *    dav_format_opaquelocktoken back into a sw_uuid_t
 *    structure.
 *
 * Returns 0 on success, 1 on failure.
 */
int dav_parse_opaquelocktoken(const char *char_token, sw_uuid_t *bin_token)
{
    int i;
 ...
 *    > 0 : a > b
 */
int dav_compare_opaquelocktoken(const sw_uuid_t a, const sw_uuid_t b)
{
    return memcmp(&a, &b, sizeof(sw_uuid_t));
}

/* format_uuid_v1 -- make a UUID from the timestamp, clockseq, and node ID */
static void format_uuid_v1(sw_uuid_t * uuid, unsigned16 clock_seq,
			   sw_uuid_time_t timestamp, uuid_node_t node)
{
...
static void get_current_time(sw_uuid_time_t * timestamp)
{
    sw_uuid_time_t         time_now;
    static sw_uuid_time_t  time_last;
...
static void get_system_time(sw_uuid_time_t *sw_uuid_time)
{
    ...
    *sw_uuid_time = (tp.tv_sec * 10000000) + (tp.tv_usec * 10) +
