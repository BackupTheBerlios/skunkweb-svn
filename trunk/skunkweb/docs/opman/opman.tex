...
\item The mxBase Python extension package, available from www.egenix.com.
...
\subsubsection{The \texttt{ae_component} service}
\begin{argdesc}
\item[documentRoot]
...
\end{description}
Default is 0
\item[runOutOfCache] causes the system to not actually read any of the
source files as long as there is a compiled version in the cache,
even if they change.
\item[dontCacheSource] causes the system to not include any source
code in the compile cache files.
\end{argdesc}



\subsubsection{The \texttt{aecgi} Service}
\begin{argdesc}
...
\subsubsection{The \texttt{fcgiprot} service}
\begin{argdesc}
...
\subsubsection{The \texttt{httpd} Service}
\begin{argdesc}
...
\subsubsection{The \texttt{requestHandler} Service}
\begin{argdesc}
...
\subsubsection{The \texttt{mysql} Service}
\begin{argdesc}
...
\subsubsection{he \texttt{oracle} Service}
\begin{argdesc}
...
\subsubsection{The \texttt{postgresql} Service}
\begin{argdesc}
...
\subsubsection{The \texttt{remote} Service}
\begin{argdesc}
...
\subsubsection{The \texttt{sessionHandler} Service}
\begin{argdesc}
\item[SessionTimeout] 
...
\subsubsection{The \texttt{templating} Service}
\begin{argdesc}
\item[errorTemplate] \nonscope
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Parameters for the \texttt{MailServices} service:
The following parameters control how mail is sent from SkunkWeb. They are used
by the \texttt{sendmail} function from \texttt{templating.MailServices}
which in turn is used by the \texttt{sendmail}-tag.

\begin{argdesc}
\item[MailMethod] The method by which to send the mail.  The default value is
\texttt{'sendmail'}.

Possible values for MailMethod:
\begin{description}
\item[sendmail] ~\\ 
    This is the standard value for MailMethod. The
    mail is sent to the program configured by
    \texttt{SendmailCommand}. It should be sendmail or a program with
    similar functionality. The sendmail function opens a pipe to the
    configured command and catches it's stderr output.  The envelope
    sender is set by using the parameter \texttt{-f} and the
    (envelope-) receivers are given on the commandline.
\item[qmail] ~\\
    This value is used for delivering mail through qmail. The mail is
    sent to the program configured by \texttt{QmailInject}. It should
    be the qmail program qmail-inject or new-inject from
    D.J.~Bernsteins mess822 package. The sendmail function opens a
    pipe to the configured command and catches it's stderr output.
    The envelope sender is set by using the parameter \texttt{-f} and
    the (envelope-) receivers are given on the commandline.\\ You may
    notice that this sounds very similar to the description for the
    value \texttt{sendmail}~- that's absolutely right. At the moment
    there is no real difference. It is planned to support per receiver
    and per message VERP (see the qmail homepage for details). When
    this is implemented, the both methods will be different.
\item[relay] ~\\
    This value sets the MailService so that mail is delivered through
    SMTP to a Relay-Host (also called SmartHost sometimes). The
    RelayHost is configured by the parameter \texttt{MailHost}. SMTP
    authentication or TLS are not supported.
\end{description}
\item[MailHost] If using the \texttt{relay} mail method the relay host to send it to.  Default is \texttt{'localhost'}.
\item[SendmailCommand] If using the \texttt{sendmail} mail method, the
path to the sendmail (or sendmail compatible) program.  Defaults to
\texttt{'/usr/lib/sendmail'}.
\item[FromAddress] 
The fallback sender (used in
the \texttt{From:} header and as envelope~sender) if there is no sender given
in the call to the \texttt{sendmail} function. The \texttt{sendmail} function 
\textbf{does not}
accept an empty envelope~sender~- these are used for bounces and it's 
not
likely that a web application has to send bounces.\\
\textbf{Please note:} A major annoyance to mail administrators is the 
use
of invalid envelope~senders. Many of them block sites generally if they
are lazy/stupid regarding envelope~senders or they do intensive checks 
on
them. So it's not only good behaviour to set a valid envelope sender,
it's also in your own interest to do so. The fallback is your defense
against lazy programming, so use it.  Defaults to \texttt{"root@localhost"}.
\item[QmailInject] If using the \texttt{qmail} mail method, the path
to the injector program.  Defaults to
\texttt{'/var/qmail/bin/qmail-inject'}.
\end{argdesc}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{The \texttt{userdir} Service}
\begin{argdesc}
