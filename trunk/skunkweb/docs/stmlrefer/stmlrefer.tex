...
Sometimes a debugging feature of SkunkWeb, component comments, can be
a nuisance when you are spooling the contents of component calls.  To get
around this you can temporarily reset the value of the configuration
variable \texttt{componentCommentLevel} that controls this feature:
...
The \texttt{comments} argument can take the same values as
\texttt{componentCommentLevel} itself: 0 (no comments) through 3 
(verbose comments).
...
% For a more practical explanation and some real life examples
% see the document ``Sending mail with SkunkWeb''.
