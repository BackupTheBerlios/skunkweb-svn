class Action: #lawsuit?
    def __init__(self, action=None, *args):
        """action/args can be one of ('GOTO', formname), ('PUSH', formname)
        or ('POP', dontcare)
        """
        self.args = args 
        self.action = action
...
#as stolen from the CONNECTION method of the same name
def extract_args(self, argsrc, args, kwargs):
    """
...
####################
#generic xhtml widgets

class Element:
    def __init__(self, tagname, kvpairs={}, pretext='', posttext=''):
    ...
    def asHTML(self, state, ns):
        return '%s<%s %s/>%s' % (
            ...
            self.tagname,
            ' '.join([
               ((v is None) and ('%s' % k) or ("%s=%s" % (k,v)))
               for k,v in self.kvpairs.items()]),
            self.posttext
            )

class ElementContainer(Element):
    def __init__(self, tagname, kvpairs, pretext, posttext, kids):
        Element.__init__(self, tagname, kvpairs, pretext, posttext)
        self.kids = kids
    ...
    def asHTML(self, state, ns):
        ...
        for i in self.kids:
            sl.append(i.asHTML(state, ns))
        sl.append('</%s>%s' % (self.tagname, self.posttext))
        return ''.join(sl)
...
class Option(ElementContainer):
    def __init__(self, display, value, isDefault = None):
        kvpairs = {'value':value}
        if isDefault:
            kvpairs['selected'] = None
        ElementContainer.__init__(self, 'option', kvpairs, '', '',
                                  [Text(display)])
            
class Selector(ElementContainer):
    def __init__(self, choices, default, kvpairs={}, pretext='', posttext=''):
        kids = []
        for disp, val in choices:
            kids.append(Option(disp, val, val == default))
        ElementContainer.__init__(self, 'select', kvpairs, pretext,
                                  posttext, kids)
