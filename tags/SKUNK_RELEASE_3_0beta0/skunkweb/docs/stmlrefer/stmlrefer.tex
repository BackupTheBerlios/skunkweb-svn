...
\texttt{.html}), all STML component documents end with the file extension
\texttt{.comp} (or \texttt{.dcmp} for data components; more about
those later). 
...
\texttt{/foo/bar.comp} or \texttt{foo.comp}. If the path is
...
\begin{verbatim}<:include /header.comp:>
<:include foo.comp:>
\end{verbatim}
...
and the STML component document \texttt{foo.comp} 
...
extension \texttt{.pycomp}.  Use them in the \texttt{<:include:>} or
\texttt{<:component:>} \doref{tagcomponent} tags just as you would
regular \texttt{.comp} component files.  See Python Components,
...
setting the SkunkWeb templating configuration variable
...
\begin{verbatim}<:import name [items]:>\end{verbatim}
...
\begin{verbatim}<:img path [noescape] [extra_args]:>\end{verbatim}
...
and it defaults to the address in the SkunkWeb \texttt{templating} personality's
...
function in the \texttt{templating} module called, aptly enough,
...
\label{pythonincl}
The Python include component extension is \texttt{.pycomp}.
...
in 
Python
unfortunately, but this can be done by setting the global variable
named \texttt{__expiration} to the number of seconds past the Epoch
(12:00am 1/1/1970 UTC) at which this component will expire.  Sounds
nasty, well of course there is a simple way to compute this value!  In
the \texttt{templating} (which is already in the global namespace,
no need to import it), there are two functions which mimic STML's
...
exception value being the return value.  
