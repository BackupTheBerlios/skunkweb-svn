...
def getConnection(connUser):
    ...
    if not _connections.has_key(connectParams):
        try:
            _connections[connectParams] = apply(MySQL.connect,
                                                connectParams[0])
            _connections[connectParams].selectdb(connectParams[1])
        except MySQL.error:
...
def sqlExec(connUser, stmt, start = 0, end = None, fudge = 0):
    ...
    connUser - use the connection named by it values
    ...
    """
    cur = getConnection(connUser).query(stmt)
    try:
        ret = cur.fetchrows()
    except MySQL.error:
